defmodule TasktracerWeb.TimeblockControllerTest do
  use TasktracerWeb.ConnCase

  alias Tasktracer.Social
  alias Tasktracer.Social.Timeblock

  @create_attrs %{endtb: "some endtb", starttb: "some starttb"}
  ...
  describe "update timeblock" do
    ...
    test "renders timeblock when data is valid", %{conn: conn, timeblock: %Timeblock{id: id} = timeblock} do
      ...
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "endtb" => "some updated endtb",
        "starttb" => "some updated starttb"}
    end
